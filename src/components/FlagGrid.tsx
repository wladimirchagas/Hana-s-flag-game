...
export type FlagGridProps = {
   ...
   *  specific tourism logo; `newsAgencyId` is the specific national news agency;
   *  `olympicCommitteeId` is the specific National Olympic Committee (a non-UN IOC member entity);
   *  `worldMapCode` is that entity's own ISO alpha-2 code. */
  onSelect: (
    id: string,
    crestId?: string,
    airlineId?: string,
  ...
  /** Currently selected tourism logo ID (if any) */
...
/** TI CPI map score bands — cleanest first. */
const CPI_ORDER: Record<string, number> = {
  "90–100": 1,
  "80–89": 2,
  "70–79": 3,
  "60–69": 4,
...
const RSF_PRESS_ORDER: Record<string, number> = {
  Good: 1,
  Satisfactory: 2,
  Problematic: 3,
  Difficult: 4,
...
const GPI_ORDER: Record<string, number> = {
  "Very High": 1,
  High: 2,
  Medium: 3,
  Low: 4,
...
/** Lowy Global Diplomacy Index post-count bands — largest networks first. */
const GDI_ORDER: Record<string, number> = {
  "250+": 1,
  "200–249": 2,
  "150–199": 3,
  "100–149": 4,
...
/** Reuters Digital News Report trust % bands — highest trust first. */
const DIGITAL_NEWS_ORDER: Record<string, number> = {
  "90–100%": 1,
  "80–89%": 2,
  "70–79%": 3,
  "60–69%": 4,
...
export function FlagGrid({
  ...
  const chooseGroupMode = (mode: GroupMode) => {
    ...
    try {
      ...
      /* ignore — localStorage best-effort */
    }
  };
  ...
  const displayEntries = useMemo((): FlagListEntry[] => {
    ...
    if (effectiveContentType === "tourismlogo") {
      const codeToEntry = new Map(entries.map((e) => [e.id, e]));
    ...
    if (effectiveContentType === "newsagency") {
      const codeToEntry = new Map(entries.map((e) => [e.id, e]));
  ...
  const groups = useMemo(() => {
    ...
    } else if (groupMode === "imd-competitiveness") {
      for (const e of sorted) {
        const code = (e.selectId || e.id || e.worldMapCode || "").toUpperCase();
    ...
    } else if (groupMode === "digital-news") {
      for (const e of sorted) {
        const code = (e.selectId || e.id || e.worldMapCode || "").toUpperCase();
    ...
    list.sort(([a], [b]) => {
      ...
      if (groupMode === "gdi") {
        const oa = GDI_ORDER[a] ?? 99;
        const ob = GDI_ORDER[b] ?? 99;
        if (oa !== ob) return oa - ob;
      }
