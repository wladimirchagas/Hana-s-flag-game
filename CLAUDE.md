...
## National-flags tab: a country's own flags are dated images — source them, and share them with the eras — hard rule, do not override without approval

**The Learn-mode "National flags" tab (`src/components/NationalFlagGrid.tsx`, data
...
`public/national-flags/`) shows a country's OWN flags: its historical national flags newest-first with the
current one at the top, plus any additional officially recognised flags, military service flags, maritime
ensigns/jacks, head-of-state standards, civil/state variants and indigenous flags. A category a country has
none of is hidden entirely. Selecting a card opens the flag's own widget below the country fact-sheet and
...
6. **A flag that cannot be freely licensed is OMITTED with a written reason**, in the country's `omitted`
   array (Australia's Torres Strait Islander Flag is still under copyright, so no free file exists and
   en.wikipedia itself shows a placeholder). Never bundle a non-free image, and never approximate one.
7. **Every meaning is sourced.** `NATIONAL_FLAG_MEANINGS` feeds the SAME `FlagMeaning` component the
   national/subnational/city flags use, under the same rule ("Flag-meaning explanations must be sourced and
   must separate myth from fact"). A flag with no sourced symbolism ships with no explainer.
...
9. **Verify in the running app** (the mandatory visual-verification rule applies): open a country's
   National flags tab, confirm the sections present are only the ones it has, that the current flag heads
