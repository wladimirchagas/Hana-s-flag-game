...
async function run(code,needle){
  for(let t=0;t<3;t++){
    ...
    try{
      await p.goto(`http://localhost:5173/learn?country=ID&subdivisions=1&sub=${code}`,{waitUntil:"load",timeout:30000});
      let ok=false;
...
for(const [cd,nd] of [["ID-SU","deli tobacco"],["ID-SN","pinisi"],["ID-YO","diponegoro"],["ID-JT","five-day battle"]]) await run(cd,nd);
await b.close();
