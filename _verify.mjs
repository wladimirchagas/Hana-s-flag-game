...
async function run(code,needle){
  for(let t=0;t<3;t++){
    ...
    try{
      await p.goto(`http://localhost:5173/learn?country=IT&subdivisions=1&sub=${code}`,{waitUntil:"load",timeout:30000});
      let ok=false;
...
for(const [cd,nd] of [["IT-AQ","immota manet"],["IT-AR","rearing horse"],["IT-AL","barbarossa"],["IT-BI","bear"]]) await run(cd,nd);
await b.close();
