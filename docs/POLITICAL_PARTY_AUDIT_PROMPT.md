...
| `inPower` | Current cabinet / government benches from an official source | Has the government changed, a coalition collapsed, a confidence vote fallen, or a party left/joined the governing bloc? |
| `inExecutive` | The ministerial list | Does this party hold a cabinet portfolio **now**? Distinct from `headOfGovernment`. Distinguish "supports the government" from "sits in it". |
| `headOfGovernment` | The current head of government (president where that office is HoG; prime minister otherwise) | Does **this** party supply the HoG **today**? At most one party per country. Never the head of state merely for being HoS, and never every cabinet partner. Drives the "Exec power" badge. |
| `chambers` / `PARTY_LEGISLATURES` | Each chamber's own composition page | In a bicameral country, does **this party** hold more than half of **this chamber**? `inPower` is the coalition, not a House/Senate majority. A hung house has no majority badge. |
