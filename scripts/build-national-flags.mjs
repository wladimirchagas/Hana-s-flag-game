...
//   • `historical` is emitted newest-first (the most recent superseded flag at the
//     top), which is the order the tab shows it in. The CURRENT flag is never in
//     this section — it lives in `official`.
...
for (const cc of countries) {
  ...
  for (const e of ordered) {
    ...
    // now (never duplicated into the historical timeline), so only that one card
    // carries the badge.
    //
...
// sub-national ones — its historical national flags (newest first, older superseded
// flags only), any additional flags with official national status, its military
