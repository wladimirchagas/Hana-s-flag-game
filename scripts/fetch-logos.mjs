...
async function main() {
  for (const a of AIRLINES) {
    const outPath = path.join(PUBLIC_DIR, a.cc, `${a.id}.svg`);
    const pngPath = path.join(PUBLIC_DIR, a.cc, `${a.id}.png`);
    if ((fs.existsSync(outPath) && fs.statSync(outPath).size > 500) ||
        (fs.existsSync(pngPath) && fs.statSync(pngPath).size > 500)) {
    ...
    const isPng = /\.png$/i.test(dlUrl) || /\.png$/i.test(file);
    const targetPath = isPng ? pngPath : outPath;
    fs.mkdirSync(path.dirname(targetPath), { recursive: true });

    try {
      await sleep(600);
      const resp = await fetch(dlUrl, { headers: HEADERS });
      const buf = Buffer.from(await resp.arrayBuffer());
      if (buf.length < 200 || buf.toString("utf8", 0, 40).includes("policy")) {
        console.warn(`[BLOCKED/SMALL] ${a.id}: ${buf.toString("utf8", 0, 60)}`);
      } else {
        fs.writeFileSync(targetPath, buf);
        console.log(`[OK] ${path.relative(PUBLIC_DIR, targetPath)} (${buf.length} bytes)`);
      }
    } catch (e) {
