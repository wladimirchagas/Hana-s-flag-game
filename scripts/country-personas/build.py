...
if __name__ == "__main__":
    ...
    def clean(o):
        """JSON has no NaN: a statistic with no observed members is null (never a guess)."""
        ...
        if isinstance(o, np.bool_):
            return bool(o)
        return o
    OUT_MODEL.write_text(json.dumps(clean(model), indent=1, ensure_ascii=False, allow_nan=False) + "\n")
    OUT_PROFILE.write_text(json.dumps(clean({"world": WORLD, "personas": profiles}), indent=1, ensure_ascii=False, allow_nan=False) + "\n")
    ...
    L = ["# Country Personas v2 — research build report", "",
         ...
         "Research build (draft). The edition the app ships is frozen from it in `scripts/data/country-personas-model.json`; see the ledger._", "",
