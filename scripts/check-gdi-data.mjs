...
for (const [rel, needle] of uiFiles) {
  ...
  if (rel.endsWith("EntitySummary.tsx")) {
    if (!src.includes('case "gdi"') || !src.includes("getDemocracyIndexMenuGroups")) {
      errors.push(
        `${rel}: must render gdi via formatIndexValue + getDemocracyIndexMenuGroups`,
      );
    }
  }
