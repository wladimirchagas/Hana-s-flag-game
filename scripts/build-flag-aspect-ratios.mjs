 ...
 * public/flags (e.g. "au.svg", "sub/MY/MY-13.svg", "mf.png").
 *
 * WHY THIS EXISTS
 * ---------------
 * The Learn-mode flag overlay (WorldProgressMap / SubdivisionMap) fills each
 * landmass by tiling its flag into an SVG <pattern> and painting the polygon
 * with it. Each pattern tile must be sized to the flag's TRUE aspect ratio so
 * that `preserveAspectRatio="xMidYMid meet"` fills the tile exactly (no
 * letterbox) and the seamless tiling covers the whole landmass. When the tile
 * was hard-coded to 3:2, every flag that is NOT 3:2 (2:1 flags like Australia,
 * Canada, the UK, and Malaysia's state flags; 1:1 flags like Switzerland) got
 * letterboxed inside its tile, leaving the top/bottom (or side) edges of the
 * landmass uncovered — the "portion of some landmasses not covered" bug.
 *
...
async function buildMap() {
  const files = await collectFlags(FLAGS_DIR);
  const map = {};
  const skipped = [];
...
function renderFile(map) {
  ...
  return (
    ...
    `//\n` +
    `// Maps each bundled flag file (path relative to public/flags) to its\n` +
    `// real-world aspect ratio (width / height), read from the file's own\n` +
    `// viewBox / pixel dimensions. Only ratios that differ from the 3:2 default\n` +
    `// are listed; see src/lib/flagOverlayAspectRatio.ts for the lookup + default.\n` +
    `//\n` +
